...
export const dx = ({ dx_in }) => dx_in;
export const dampener = ({ dampener_in }) => dampener_in;

// functions:

// calculang determines x function dependent on inputs t_in, dx_in
export const x = ({ t_in, dx_in }) => {
...
export const y = ({ t_in, dampener_in }) => {
  if (t({ t_in }) == 0) return 50;else
  if (y({ dampener_in, t_in: t({ t_in }) - 1 }) + dy({ dampener_in, t_in: t({ t_in }) - 1 }) > 185)
  return 190; // "floor rule"
...
export const dy = ({ t_in, dampener_in }) => {
  if (t({ t_in }) == 0) return 0 * dampener({ dampener_in }) + 3;else
  if (y({ dampener_in, t_in: t({ t_in }) }) > 185)
...
export const compressed = ({ t_in, dampener_in }) => y({ t_in, dampener_in }) == 190 ? true : false;